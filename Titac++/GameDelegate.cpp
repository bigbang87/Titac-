...
void GameDelegate::setResolution(sf::Vector2u& resolution)
{
	if (resolution == m_window.getSize())
		return;
	m_window.setView(sf::View(sf::FloatRect(0, 0, (float)resolution.x, (float)resolution.y)));
...
const sf::Vector2u GameDelegate::getFixedRatioResolution(const unsigned int x, const unsigned int y)
{
	...
	if (m_resolution.y != y)
	{
		...
		res = sf::Vector2u((unsigned int)widthRatio, y);
		return res;
	}
	if (m_resolution.x != x)
	{
		...
		res = sf::Vector2u(x, (unsigned int)widthRatio);
		return res;
	}
